#include "main.h"

const color_t COLOR_RED = { 220, 20, 20 };
const color_t COLOR_GREEN = { 0,100,0 };
const color_t COLOR_BLACK = {13, 18, 23 };
const color_t COLOR_BACKGROUND = { 0,0,0 };
const color_t COLOR_YELLOW = { 255, 255, 0 };
const color_t COLOR_GOLDEN = { 212, 175, 55 };
const color_t COLOR_WHITE = { 255, 255, 255 };
const color_t COLOR_ORANGE = { 255, 69, 0 };
const color_t COLOR_DARK_RED = { 255, 0, 0 };
const color_t COLOR_BLUE = { 64, 64, 255 };
const color_t COLOR_GREY = { 128, 128, 128 };
const color_t COLOR_INDIGO = { 75,0,130 };
const color_t COLOR_DARK_BLUE = { 0,0,255 };
